...
Writing the `for` loop to go through the list of questions felt
natural once I saw how it avoided repeating the same `ask_question()`
call 5 times.

One thing that felt confusing: 
Understanding how a function can `return` a value (like 1 or 0) and
how that returned value gets added directly into `score` using
`score += ask_question(...)`.